...
async function writeRow(c) {
  ...
  switch (c.table_name) {
    ...
    case 'job_helper':  return c.op === 'insert'
                          ? ins('job_helper', Object.assign({ id: c.row_id }, p))
                          : upd('job_helper', c.row_id, p);
...
const SELECT = [
  ...
  'job_helper ( id, full_name, company, hours, hours_source, recorded_by )'
].join(', ');
...
function shapeJob(r) {
  ...
  return {
    ...
    helpers: (r.job_helper || []).map(h => ({ id: h.id, name: h.full_name, company: h.company,
      hours: h.hours != null ? Number(h.hours) : null, mine: h.recorded_by === ME.id })),
...
function overlayPending(jobs) {
  ...
  for (const c of pending()) {
    ...
    switch (c.table_name) {
      ...
      case 'job_helper': {
        j.helpers = j.helpers || [];
        const h = j.helpers.find(x => x.id === c.row_id);
        if (c.op === 'insert' && !h) j.helpers.push({ id: c.row_id, name: p.full_name, company: p.company || null,
          hours: p.hours != null ? Number(p.hours) : null, mine: true, local: true });
        else if (h && 'hours' in p) h.hours = p.hours != null ? Number(p.hours) : null;
        break;
      }
      case 'job_visit':
...
async function afterSignIn(session) {
  ...
  // Office and owner accounts belong on the board, not the crew app. Hand the
  // session over (so no second sign-in), forget it here, and go.
  if (ME && ME.role !== 'technician') {
    try {
      const tok = localStorage.getItem('wwd.auth');
      if (tok) localStorage.setItem('wwd-office', tok);
...
async function signIn(email, password) {
  ...
  // Men type their short name ("cheo"); accounts live at @wwdi.local.
  // Anything that already contains an @ is used as typed.
  let login = email.trim().toLowerCase().replace(/\s+/g, '');
  if (login && !login.includes('@')) login += '@wwdi.local';
...
const T = {
  en: { v_pending:'Voice note — text will appear when there is signal', v_listening:'Listening…', v_recording:'Recording…',
        ...
        v_nothing:'Nothing recorded', v_short:'Too short', v_rv_title:'Send this voice note?', v_rv_send:'Send', v_rv_del:'Delete', v_rv_nowords:'No words were picked up. You can still send the recording.', v_rv_deleted:'Voice note deleted',
        ...
        offline:'Working offline', role:'Technician', role_office:'Office', crew:'On this job', helpers:'Helpers', h_none:'No helpers on this job.', h_add:'+ Add helper', h_title:'Add a helper', h_sub:'Someone not on the crew list who worked on this job.', h_name:'Helper’s name', h_co:'Company (if subcontractor)', h_hours:'Hours on this job', h_nohours:'hours not entered', h_save:'Add helper', h_bad:'Hours must be more than 0 and no more than 24.', h_needname:'Type the helper’s name.', cl_helpers:'Helper hours', cl_helpers_sub:'Total hours each helper worked on this job.',
        ...
        kind_title:'What is this photo?', k_before:'Before', k_after:'After', k_receipt:'Receipt', k_other:'Other', k_ticket_scan:'Paper ticket',
  ...
  es: { v_pending:'Nota de voz — el texto aparecerá cuando haya señal', v_listening:'Escuchando…', v_recording:'Grabando…',
        ...
        v_nothing:'No se grabó nada', v_short:'Muy corto', v_rv_title:'¿Enviar esta nota de voz?', v_rv_send:'Enviar', v_rv_del:'Borrar', v_rv_nowords:'No se captaron palabras. Puede enviar la grabación de todos modos.', v_rv_deleted:'Nota de voz borrada',
        ...
        offline:'Sin conexión', role:'Técnico', role_office:'Oficina', crew:'En este trabajo', helpers:'Ayudantes', h_none:'No hay ayudantes en este trabajo.', h_add:'+ Agregar ayudante', h_title:'Agregar ayudante', h_sub:'Alguien que no está en la lista del equipo y trabajó en este trabajo.', h_name:'Nombre del ayudante', h_co:'Compañía (si es subcontratista)', h_hours:'Horas en este trabajo', h_nohours:'horas sin anotar', h_save:'Agregar', h_bad:'Las horas deben ser más de 0 y no más de 24.', h_needname:'Escriba el nombre del ayudante.', cl_helpers:'Horas de ayudantes', cl_helpers_sub:'Total de horas que cada ayudante trabajó en este trabajo.',
        ...
        kind_title:'¿Qué es esta foto?', k_before:'Antes', k_after:'Después', k_receipt:'Recibo', k_other:'Otro', k_ticket_scan:'Ticket en papel',
...
function fmtHrs(h) { return (Math.round(h * 100) / 100) + ' h'; }
function paintHelpers() {
  const j = OPEN; j.helpers = j.helpers || [];
  document.getElementById('d_helpers').innerHTML = j.helpers.length
    ? j.helpers.map(h => '<div class="man"><span class="av">' + esc(initials(h.name)) + '</span>' +
        '<span>' + esc(h.name) + (h.company ? ' <span class="hco">· ' + esc(h.company) + '</span>' : '') + '</span>' +
        '<span class="st' + (h.hours != null ? ' on' : '') + '">' + esc(h.hours != null ? fmtHrs(h.hours) : t('h_nohours')) + '</span></div>').join('')
    : '<div class="hnone">' + esc(t('h_none')) + '</div>';
}
function validHours(v) {
  if (v === '' || v == null) return null;
  const n = Number(String(v).replace(',', '.'));
  return n > 0 && n <= 24 ? Math.round(n * 100) / 100 : NaN;
}
function openHelper() {
  ['h_name_in', 'h_co_in', 'h_hours_in'].forEach(i => { document.getElementById(i).value = ''; });
  document.getElementById('h_err').textContent = '';
  openModal('helpermodal');
  setTimeout(() => document.getElementById('h_name_in').focus(), 50);
}
function saveHelper() {
  const j = OPEN; const err = document.getElementById('h_err');
  const name = document.getElementById('h_name_in').value.trim();
  const co = document.getElementById('h_co_in').value.trim();
  const hrs = validHours(document.getElementById('h_hours_in').value);
  if (!name) { err.textContent = t('h_needname'); return; }
  if (Number.isNaN(hrs)) { err.textContent = t('h_bad'); return; }
  const id = uuid();
  j.helpers = j.helpers || [];
  j.helpers.push({ id, name, company: co || null, hours: hrs, mine: true, local: true });
  queue({ table: 'job_helper', row_id: id, op: 'insert', wo: j.id,
    payload: { work_order_id: j.id, full_name: name, company: co || null, is_subcontractor: !!co,
               hours: hrs, hours_source: hrs != null ? 'entered_by_lead' : 'not_recorded', recorded_by: ME.id } });
  shutModal('helpermodal');
  paintHelpers(); saveSnapshot();
  if (document.getElementById('closemodal').classList.contains('open')) paintCloseHelpers();
}
/* Helper hours on the close-out screen: filled in from what is known,
   saved only when he actually closes the job. */
function paintCloseHelpers() {
  const j = OPEN; const box = document.getElementById('cl_helpers');
  const hs = j.helpers || [];
  box.innerHTML = '<div class="lbl">' + esc(t('cl_helpers')) + '</div>' +
    (hs.length ? '<p class="hsub">' + esc(t('cl_helpers_sub')) + '</p>' + hs.map(h =>
      '<label class="hrow"><span>' + esc(h.name) + '</span><input class="inp hin" type="number" inputmode="decimal" min="0.25" max="24" step="0.25" data-h="' + h.id + '" value="' + (h.hours != null ? h.hours : '') + '" aria-label="' + esc(h.name) + '"></label>').join('')
      : '<p class="hsub">' + esc(t('h_none')) + '</p>') +
    '<button class="act ghost small" id="cl_addhelper">' + esc(t('h_add')) + '</button><div class="err" id="cl_herr"></div>';
  document.getElementById('cl_addhelper').onclick = openHelper;
}
function commitCloseHelpers(j) {
  const inputs = [...document.querySelectorAll('#cl_helpers .hin')];
  for (const i of inputs) if (Number.isNaN(validHours(i.value))) { document.getElementById('cl_herr').textContent = t('h_bad'); i.focus(); return false; }
  for (const i of inputs) {
    const h = (j.helpers || []).find(x => x.id === i.dataset.h); if (!h) continue;
    const v = validHours(i.value);
    if (v === h.hours) continue;
    h.hours = v;
    queue({ table: 'job_helper', row_id: h.id, op: 'update', wo: j.id,
      payload: { hours: v, hours_source: v != null ? 'entered_by_lead' : 'not_recorded' } });
  }
  return true;
}
...
function paintAtts() {
  ...
  document.getElementById('d_atts').innerHTML = j.attachments.map(a => {
    ...
    return '<div class="att"><span class="k">' + esc(label) + '</span>' +
      (a.caption && a.kind !== 'ticket_scan' ? '<span>' + esc(a.caption) + '</span>' : '') +
...
onReady(() => {
  ...
  function voiceFinish() {
    ...
    if (!OPEN) return;
    // Nothing is saved or sent yet. He reviews it first: play it back,
    // read what the phone heard, then Send or Delete.
    vReview({ text, blob, secs, job: OPEN });
  }

  function voiceCommit(r) {
    const job = r.job;
    const id = uuid(), now = iso();
    const text = r.text, blob = r.blob, secs = r.secs;
    ...
    const path = blob ? job.id + '/audio/' + id + '.' + extFor(blob.type) : null;

    job.notes.push({ id, author_id: ME.id, who: ME.full_name, at: now, lang: LANG,
      original: text || null, translated: null, audio_secs: secs, audio: !!blob,
      awaiting_text: pendingText, local: true });

    queue({ table: 'note', row_id: id, op: 'insert', wo: job.id,
      payload: { work_order_id: job.id, author_id: ME.id, source: 'voice',
  ...
  function vReview(r) {
    let el = document.getElementById('vreview');
    if (!el) { el = document.createElement('div'); el.id = 'vreview'; document.body.appendChild(el); }
    const audioUrl = r.blob ? URL.createObjectURL(r.blob) : null;
    const secs = r.secs, mm = Math.floor(secs / 60) + ':' + String(secs % 60).padStart(2, '0');
    el.innerHTML =
      '<div class="vbox">' +
        '<div class="vrtitle">' + esc(t('v_rv_title')) + ' <span class="vrlen">' + mm + '</span></div>' +
        '<div class="vtext' + (r.text ? '' : ' dim') + '">' + esc(r.text || t('v_rv_nowords')) + '</div>' +
        (audioUrl ? '<audio controls src="' + audioUrl + '"></audio>' : '') +
        '<div class="vrbtns">' +
          '<button type="button" class="act ghost" id="vr_del">' + esc(t('v_rv_del')) + '</button>' +
          '<button type="button" class="act go" id="vr_send">' + esc(t('v_rv_send')) + '</button>' +
        '</div>' +
      '</div>';
    el.classList.add('on');
    const close = () => { el.classList.remove('on'); const a = el.querySelector('audio'); if (a) a.pause();
      if (audioUrl) setTimeout(() => URL.revokeObjectURL(audioUrl), 1000); el.innerHTML = ''; };
    document.getElementById('vr_send').onclick = () => { close(); voiceCommit(r); };
    document.getElementById('vr_del').onclick = () => { close(); toast(t('v_rv_deleted')); };
  }
...
function wireClose() {
  ...
  document.getElementById('cl_sign').onclick = () => {
    ...
    const j = OPEN;
    if (!commitCloseHelpers(j)) return;
  ...
  document.getElementById('cl_nosig').onclick = () => { if (!commitCloseHelpers(OPEN)) return; shutModal('closemodal'); openModal('reasonmodal'); };
  document.getElementById('h_cancel').onclick = () => shutModal('helpermodal');
  document.getElementById('h_save').onclick = saveHelper;
  document.getElementById('addhelper').onclick = openHelper;
