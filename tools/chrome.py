...
# the footer's ink and the hover color without a second asset. The third
...
def licence_line(sep=" &middot; ", label=True):
    ...
    markup already says "License" — the property managers table does."""
    live = [(k, v) for k, v in LICENCES if v.strip()]
    if not live:
        return "License no. pending"
    body = sep.join("%s %s" % (k, v) for k, v in live)
    return ("License " + body) if label else body
...
<header class="hdr">
  <div class="wrap hdr-in">
    <a class="brand" href="index.html">
      <span class="mark mark-wwd" aria-hidden="true">
        <img src="assets/brand/mark-wwd.png" alt="" width="93" height="34" decoding="async">
        <img class="ngt" src="assets/brand/mark-wwd-night.png" alt="" width="93" height="34" decoding="async">
      </span>
      <span class="wm">
